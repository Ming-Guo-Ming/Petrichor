...
def basic_stats_and_aggregates(
    ...
    if sum_df is not None and len(sum_df.columns) > 0:
        ...
        monthly_sum = sum_df.resample("MS").sum()
        yearly_sum = sum_df.resample("YS").sum()
...
def write_timeseries_outputs(
    ...
    def _prepare_export_frame(frame: pd.DataFrame, decimals: int = 3) -> pd.DataFrame:
        out = frame.copy()
        out = out.drop(columns=["NET_SCALED_FLAG","MOD", "NET_MIN_SECONDS","N_corr", "FLAG"], errors="ignore")
...
def build_run_metadata(
    *,
    ...
    metadata["parameters"] = {
        ...
        "density": _cfg_get(run_cfg, "density", 2.65),
...
def run_analysis_exports(
    *,
    ...
    try:
        from petrichor.processing import write_site_summary
