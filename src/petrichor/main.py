...
from petrichor.processing import *
from petrichor.analysis import run_analysis_exports
from petrichor.visualization import *
import petrichor.processing as processing
...
def _read_url_input_data(cfg: dict) -> tuple[pd.DataFrame, dict, Path]:
    ...
    header_rows = int(_cfg_get(cfg, "header_rows", 2))
    ...
    df, units_dict = read_local_input_data(str(downloaded), header_rows=header_rows)
...
def _discover_site_files(field_json_path: Path, cfg: dict) -> tuple[Path, Path | None]:
    ...
    # remove calibration-like files from input candidates
    input_candidates = [
        p for p in input_candidates
        if ("calib" not in p.name.lower() and "calibration" not in p.name.lower())
    ]
...
def _create_field_json_interactive(field_json_path: Path) -> dict:
    ...
    output_figures = _prompt_with_default("project.output_figures", "output/figures")
    ...
    header_rows = int(_prompt_with_default("config.header_rows", 2))
    ...
    time_col = str(_prompt_with_default("config.time_col", "TIMESTAMP"))
    timestamp_format = str(_prompt_with_default("config.timestamp_format", "auto"))
    rhov0_ref = float(_prompt_with_default("config.rhov0_ref", 0))
    bulk_density = float(_prompt_with_default("config.bulk_density", 1.43))
    density = float(_prompt_with_default("config.density", 2.65))
    ...
    config = {
        ...
        "config": {
            ...
            "timestamp_format": None if timestamp_format.strip().lower() == "auto" else timestamp_format,
...
# ==============================================================================
    ...
    if data_source == "url":
        ...
        header_rows = int(_cfg_get(config, "header_rows", 2))
        df, units_dict = read_local_input_data(str(input_path), header_rows=header_rows)

    print("[INFO] Data loading complete.")

    # Check required/additional variables
    if _cfg_get(config, "required_vars", None) is not None:
        check_variables_in_data(df, config)

    # Replace missing values first
    time_col = _cfg_get(config, "time_col", "TIMESTAMP")
    timestamp_format = _cfg_get(config, "timestamp_format", None)
    df = replace_missing_with_nan(df, _cfg_get(config, "missing_value", -999))

    # Round timestamps and aggregate
    df = round_to_hour(df, time_col)

    # Ensure continuous hourly timestamps
    df = continuous_hourly_timestamps(df, time_col)

    # Normalize neutron counts by NET
    df, neutron_col_by_net, net_meta = normalize_by_net(df, config, n_col="N", net_col="NET", out_col="N_CPH")
...
# ==============================================================================
    ...
    preferred_neutron_col = neutron_col_by_net
    if preferred_neutron_col in df.columns:
        neutron_col = preferred_neutron_col
    elif _cfg_get(config, "neutron_col", None) in df.columns:
        neutron_col = _cfg_get(config, "neutron_col")
        ...
        if numeric_first is None:
            raise ValueError("[NEUTRON] Could not find a numeric neutron count column.")
        neutron_col = numeric_first

    # local run config copy for station
    run_cfg = json.loads(json.dumps(config))
    if "config" not in run_cfg:
        run_cfg["config"] = {}
    run_cfg["config"]["neutron_col"] = neutron_col
    ...
    try:
        ...
        if (beta_B is None or p0_ref is None) and Rc is not None and lat is not None:
            ...
            try:
                _save_json(field_json_path, config)
                print(f"[INFO] Updated {field_json_path.name} with beta_B={beta_B:.5f}, p0_ref={p0_ref:.2f}")
            except Exception as e:
...
# ==============================================================================
    ...
    bd_for_sm = float(_cfg_get(run_cfg, "bulk_density", 1.43))
    density_for_sm = float(_cfg_get(run_cfg, "density", 2.65))
    ...
    if sm_max_cfg in (None, "", "null"):
        ...
        sm_max = round(sm_max_calc(bd=bd_for_sm, density=density_for_sm), 4)
...
def main():
    ...
    print("Usage:")
    print("  python -m src.main --site input/001/site001.json")
